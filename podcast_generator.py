...
def truncate_content_smart(content, max_chars=15000):
    ...
    if last_sentence_pos > max_chars * 0.7:
...
def generate_podcast_script(news_title, news_content, requirements=None, model=None, max_retries=2):
    """
    Generates a podcast script from news content using Claude CLI.
    """
    if model is None:
        model = os.getenv("PODCAST_MODEL", "sonnet")
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            print(f"🤖 Claude ({model})로 대본 생성 중... (시도 {attempt + 1}/{max_retries + 1})")
            ...
            print(f"❌ Claude 오류 (시도 {attempt + 1}): {e}")
            if attempt < max_retries:
                print(f"재시도 중... ({attempt + 1}/{max_retries}) - 30초 후 재시도")
                time.sleep(30)
                continue
            else:
                return f"⚠️ Claude 오류 발생: {e}"
